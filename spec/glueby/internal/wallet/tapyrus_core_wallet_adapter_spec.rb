...
RSpec.describe 'Glueby::Internal::Wallet::TapyrusCoreWalletAdapter' do
  ...
  describe 'list_unspent' do
    ...
    subject { adapter.list_unspent(wallet_id, color_id, only_finalized) }
    ...
    context 'with label' do
      subject { adapter.list_unspent(wallet_id, color_id, true, "Glueby-Contract-Tracking") }
      it 'should call listunspent RPC with label and parse the results.' do
        expect(rpc).to receive(:listunspent).and_return(response)
        expect(subject).to eq([
          {
    ...
    context 'with unlabeled by assign :unlabeled' do
      subject { adapter.list_unspent(wallet_id, color_id, true, :unlabeled) }
    ...
    context 'with all utxos' do
      subject { adapter.list_unspent(wallet_id, color_id, true, :all) }
